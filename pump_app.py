...
def auto_close_maintenance_for_stable_pumps(project_id, stable_pumps):
    if not stable_pumps:
        return []
    maint_df = get_maintenance_events(project_id)
    if maint_df.empty:
        return []
...
def render_add_maintenance_wizard():
    ...
    if st.button("Save Maintenance Event", use_container_width=True, type="primary", disabled=not can_save):
        try:
            ...
            add_event_log_entry(f"Maintenance logged ({event_type}, {severity}) for pump(s): {', '.join(affected_pumps)}.")
            ...
            queue_confirmation("Maintenance event saved.")
            st.session_state.page = "dashboard"
            st.rerun()
        ...
        if open_like_df.empty:
            ...
            if st.button("Apply Status Update", use_container_width=True, key="maintenance_update_apply"):
                ...
                if selected_id is not None:
                    ...
                    conn.commit()
                    conn.close()
                    add_event_log_entry(f"Maintenance event #{selected_id} status updated to {new_status}.")
                    persist_event_log_for_project(project_id)
                    queue_confirmation("Maintenance status updated.")
                    st.rerun()
...
def render_add_record_wizard():
    ...
    if "record_ts" not in draft:
        draft["record_ts"] = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
