    ...
    for idx, p in enumerate(projs):
        ...
        if st.session_state.get(confirm_key, False):
            ...
            if c[6].button("DANGER Confirm Delete Project", key=f"conf_{idx}", use_container_width=True, type="primary"):
                del_conn = _get_conn(db_file)
                del_conn.execute(
                    _adapt_sql(del_conn, "DELETE FROM projects WHERE project_id=?"),
                    (p[0],),
                )
                # Always use project_id column (the legacy fallback for old SQLite DBs
                # that used project_name is skipped when Postgres is active because
                # init_legacy_db always creates the column as project_id)
                try:
                    del_conn.execute(
                        _adapt_sql(del_conn, "DELETE FROM pumps WHERE project_id=?"),
                        (p[0],),
                    )
                except Exception:
                    if not _is_pg(del_conn):
                        # SQLite backward-compat: discover actual column name via PRAGMA
                        cursor = del_conn.execute("PRAGMA table_info(pumps)")
                        cols = [info[1] for info in cursor.fetchall()]
                        actual_col = cols[1]
                        del_conn.execute(
                            f"DELETE FROM pumps WHERE {actual_col}=?", (p[0],)
                        )
